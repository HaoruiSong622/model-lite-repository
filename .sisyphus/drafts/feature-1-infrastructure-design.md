...
> **设计决策**: category 和 model_type 采用**真删除**（无 deleted 字段），删除前确保下无模型引用。
...
> **部分索引**: `WHERE (deleted = FALSE)` 确保软删除后名称可被复用。唯一约束维度为 (name, category_id, type_id)。
...
erDiagram
    ...
    model {
        ...
        VARCHAR author "模型作者"
        VARCHAR series_name "系列名称"
        BIGINT model_size "模型大小（字节）"
...
| 表名 | 索引名 | 索引类型 | 索引字段 | 说明 |
|------|--------|----------|----------|------|
| model | idx_model_category | B-tree | category_id WHERE deleted = FALSE | 按分类筛选 |
...
| 分类下无模型时才可删除 | category/model_type 下有模型引用时禁止删除 | 代码校验（Feature 3） |
...
> **集成测试环境**: 使用 H2 内存数据库（PostgreSQL 兼容模式），不依赖外部 PostgreSQL 实例，离线可运行。H2 覆盖 Mapper 基本 CRUD、ResultMap 映射、TypeHandler 转换等场景。PostgreSQL 特有特性（如 model 表的部分唯一约束 `WHERE deleted = FALSE`）不在 H2 测试覆盖范围内，由 DDL review 和生产环境验证保障。

#### 数据库连接验证

**Given**:
- H2 内存数据库已初始化
- category 表中已有 name='TextGeneration' 的记录

**When**:
- 执行 `INSERT INTO category (id, name) VALUES (UUID(), 'TextGeneration')`
