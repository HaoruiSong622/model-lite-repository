...
| 序号 | 决策领域 | 决策内容 |
|------|----------|----------|
| 1 | 版本锁机制 | 关系表 + TTL(24小时)，支持多任务并发锁定 |
...
| 序号 | 决策领域 | 决策内容 | 权衡因素 |
|------|----------|----------|----------|
| 1 | 版本锁机制 | 关系表 + TTL(24小时) | 简单可靠，支持多任务并发锁定；需定期清理过期锁 |
...
**版本锁 TTL**：采用 TTL 自动过期机制，防止因异常导致锁无法释放的问题，同时减少数据库存储压力。
...
**版本锁不支持公平锁**：当前版本锁实现不支持公平锁语义，高并发场景下可能出现锁饥饿问题。低概率事件，可接受。
...
|----------|----------|--------|----------|----------|
| REQ-MODEL-001 | 模型创建 | 高 | ModelService, ModelRepository | 创建模型及第一个版本，校验名称唯一性 |
| REQ-MODEL-002 | 模型查看 | 高 | ModelService, ModelApi | 查询模型详情及版本列表 |
| REQ-MODEL-003 | 模型修改 | 高 | ModelService | 修改描述、分类、类型、标签 |
| REQ-VERSION-001 | 模型版本创建 | 高 | VersionService | 自动分配版本号，支持纳管/上传 |
| REQ-VERSION-002 | 模型版本查看 | 高 | VersionService | 返回版本详情、权重路径、状态等 |
...
| REQ-CATEGORY-001 | 模型分类管理 | 高 | ClassificationService | 两级分类体系管理 |
...
| REQ-M2M-002 | 机机接口 — 训练权重归档 | 高 | ArchiveApi | 创建版本，分配路径 |
