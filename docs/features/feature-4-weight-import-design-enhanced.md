...
| rsync 完成 | Informer Watch → `TaskReconciler.onJobCompleted()` | Job 完成事件与超时检测竞态 | 高 |
| rsync 失败 | Informer Watch → `TaskReconciler.onJobFailed()` | 同上 | 高 |
| Job 超时 | K8s `activeDeadlineSeconds` 触发 Pod 终止 → Informer Watch | 超时事件与正常完成事件竞态 | 高 |
...
#### 3.8.3 完整状态转换守卫矩阵（6 状态 × 9 触发源）

下表覆盖全部 54 个状态-触发源组合。每个单元格格式为：
...
| 当前状态 ↓ \ 触发源 → | 用户暂停 | 用户恢复 | 用户取消 | Job 校验通过 | Job 校验失败 | rsync 完成 | rsync 失败 | Job 超时 | TaskReconciler 超时恢复 |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **Pending** | ❌ 0102035 | ❌ 0102035 | **→ Cancelled** | **→ Running** | **→ Failed** | ❌ 0102035 | ❌ 0102035 | **→ Failed** | **→ Pending**（重建 Job） |
| **Running** | **→ Paused** | ❌ 0102035 | **→ Cancelled** | ❌ 0102035 | **→ Failed** | **→ Completed** | **→ Failed** | **→ Failed** | —（进度检查） |
| **Paused** | ❌ 0102035 | **→ Pending** | **→ Cancelled** | ❌ 0102035 | ❌ 0102035 | ❌ 0102035 | ❌ 0102035 | ❌ 0102035 | **→ Pending**（重建 Job） |
| **Completed** | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | —（清理资源） |
| **Failed** | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | —（清理资源） |
| **Cancelled** | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | ❌ 0102042 | —（清理资源） |

##### 3.8.3.2 单元格详细说明

#### Pending 行（5 个允许转换，4 个拒绝）
...
| Job 超时 | **→ Failed** | 允许。K8s Job 创建后长期无法调度（如资源不足），超过 `activeDeadlineSeconds` 触发超时。错误消息记录："Job 调度超时" |
...
#### Running 行（5 个允许转换，1 个拒绝，1 个无操作）
...
| rsync 完成 | ❌ 0102035 | 同上 |
| rsync 失败 | ❌ 0102035 | 同上 |
| Job 超时 | ❌ 0102035 | Paused 状态下无活跃 Job，不存在超时概念。错误消息："任务已暂停，不存在超时" |
...
| 触发源 | 允许/拒绝 | 说明 |
|--------|----------|------|
| 所有 9 个触发源 | ❌ 0102042 | 终态任务不允许任何状态变更操作。错误消息统一为："任务已处于终态（{status}），无法执行 {action}" |
...
**守卫矩阵映射**: Pending × Job 校验失败/Job 超时 → Failed；Running × Job 校验失败/rsync 失败/Job 超时 → Failed

**说明**: `fail()` 由 `TaskReconciler` 在以下场景调用：
- Informer 感知到 Job Failed（Pending/Running → Failed）
- K8s `activeDeadlineSeconds` 超时（Pending/Running → Failed）
...
| `fail(message)` | Pending, Running | Failed | Job 校验失败 / rsync 失败 / Job 超时 | TaskReconciler |
...
1. 应用服务调用 `K8sJobService.createUploadJob()`:
   ...
   - 创建 Job（upload-{taskId}）：挂载源/目标 volume，设置 activeDeadlineSeconds=172800
...
- K8s activeDeadlineSeconds 超时（48h）→ Pod terminated，Job Failed → 进入阶段 6
- 用户暂停 → 进入阶段 7（暂停分支）
- 用户取消 → 进入阶段 7（取消分支）
...
- 任意阶段发生不可恢复错误：
  ...
  - 阶段 4：K8s Job 超时（activeDeadlineSeconds=172800 触发）
...
超时处理在**两个层面**实现：K8s 层面（硬限制）和应用层面（软监控）。

### K8s 层面超时

**配置项**: `activeDeadlineSeconds: 172800`（48 小时）

**行为**:
- K8s 在 Job 创建后开始计时
- 若 48 小时内 Job 未 Complete，K8s 自动终止 Pod，Job 状态变为 Failed
- Informer 感知 Failed 事件，TaskReconciler 调用 `uploadTask.fail("上传超时（48小时）")`

**Job 模板关键字段**:
```yaml
spec:
  backoffLimit: 0              # 不自动重试
  activeDeadlineSeconds: 172800  # 48小时超时
  ttlSecondsAfterFinished: 86400 # 完成24h后自动清理
```

**为何设为 48 小时**:
- TB 级权重文件通过 rsync 拷贝，极端场景下可能需要数小时至数十小时
- 48 小时覆盖 99.9% 的正常拷贝场景
- 超过 48 小时视为异常，强制终止以避免资源长期占用
...
| 4.执行 | status=Running | rsync拷贝+进度上报+定时更新 | 拷贝完成/失败/超时/用户干预 | Completed/Failed/Paused/Cancelled | Available/UploadFailed |
...
| K8s 48h 硬超时 + 应用软监控 | 两层超时策略互补 | K8s 兜底强制终止；应用层提前告警 |
...
public enum JobStatus {
    ...
    FAILED,       // Job 失败（exit 非 0 或 activeDeadlineSeconds 超时）
    NOT_FOUND     // Job 资源不存在
}
```
...
| **Running** | Failed | **读取日志** → `uploadTask.fail(message)` | 拷贝失败或超时；更新版本状态 → UploadFailed |
...
if (isNoProgressWarning(task)) {
    ...
    // 由 K8s Job 的 activeDeadlineSeconds（48h）兜底
}
```
